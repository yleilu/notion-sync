...
const mockSaveState = jest.fn<() => Promise<undefined>>().mockResolvedValue(undefined);
...
const mockUpdatePageContent = jest.fn<() => Promise<undefined>>().mockResolvedValue(undefined);
const mockArchivePage = jest.fn<() => Promise<undefined>>().mockResolvedValue(undefined);
...
describe('scanLocal', () => {
  it('finds .md files and directories recursively', async () => {
    ...
    expect(files).toEqual([
      { relativePath: 'readme.md', absolutePath: resolve(DIR, 'readme.md') },
      { relativePath: 'sub/nested.md', absolutePath: resolve(DIR, 'sub', 'nested.md') },
    ]);
    expect(dirs).toEqual([
      { relativePath: 'sub', absolutePath: resolve(DIR, 'sub') },
    ]);
  });
...
describe('startupSync', () => {
  ...
  it('creates fresh state when none exists and syncs files', async () => {
    ...
    mockMdToBlocks.mockReturnValueOnce([{ type: 'paragraph' }]);
    mockCreatePage.mockResolvedValueOnce('notion-page-1');
    mockGetPageMeta.mockResolvedValueOnce({ lastEditedTime: '2026-02-22T01:00:00.000Z' });

    const state = await startupSync(DIR, ROOT_PAGE);

    expect(mockLoadState).toHaveBeenCalledWith(resolve(DIR));
    ...
    expect(mockSaveState).toHaveBeenCalledWith(resolve(DIR), state);
  });
  ...
  it('skips unchanged files (matching hash)', async () => {
    ...
    expect(state.files['hello.md'].notionPageId).toBe('existing-page');
  ...
  it('archives removed files from state', async () => {
    ...
    expect(mockArchivePage).toHaveBeenCalledWith('page-to-archive');
...
describe('syncFile', () => {
  ...
  it('creates new page when file not in state', async () => {
    ...
    mockMdToBlocks.mockReturnValueOnce([{ type: 'heading' }]);
    mockCreatePage.mockResolvedValueOnce('created-page-id');
    mockGetPageMeta.mockResolvedValueOnce({ lastEditedTime: '2026-02-22T02:00:00.000Z' });

    await syncFile(filePath, DIR, state);

    expect(mockCreatePage).toHaveBeenCalledWith(ROOT_PAGE, 'new-file', [{ type: 'heading' }]);
  ...
  it('updates existing page when hash differs', async () => {
    ...
    mockMdToBlocks.mockReturnValueOnce([{ type: 'paragraph' }]);
    mockGetPageMeta.mockResolvedValueOnce({ lastEditedTime: '2026-02-22T03:00:00.000Z' });

    await syncFile(filePath, DIR, state);

    expect(mockUpdatePageContent).toHaveBeenCalledWith('existing-page-id', [{ type: 'paragraph' }]);
    expect(mockCreatePage).not.toHaveBeenCalled();
...
describe('syncDeleteFile', () => {
  it('archives page and removes from state', async () => {
    ...
    expect(mockArchivePage).toHaveBeenCalledWith('delete-page-id');
...
describe('syncFromNotion', () => {
  ...
  it('pulls changed content when lastEditedTime differs', async () => {
    ...
    mockGetPageMeta.mockResolvedValueOnce({ lastEditedTime: '2026-02-22T05:00:00.000Z' });
  ...
  it('skips unchanged files (same lastEditedTime)', async () => {
    ...
    mockGetPageMeta.mockResolvedValueOnce({ lastEditedTime: '2026-02-21T00:00:00.000Z' });
    ...
    expect(state.files['unchanged.md'].localHash).toBe('hash-1');
    expect(mockSaveState).toHaveBeenCalled();
  });
  ...
  it('handles errors per file without stopping others', async () => {
    ...
    mockGetPageMeta
      ...
      .mockResolvedValueOnce({ lastEditedTime: '2026-02-20T00:00:00.000Z' });
    ...
    expect(state.files['error-file.md'].localHash).toBe('hash-e');
    expect(mockSaveState).toHaveBeenCalled();
